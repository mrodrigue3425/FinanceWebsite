...
std::vector<double> px_vec(std::vector<double> TC_vec, std::vector<double> r_vec,
                           std::vector<int> K_vec, std::vector<int> d_vec);
...
TEST(f_primeTest, NonZero) {
    ...
    for (int i = 0; i < num_test; i++) {
        const double fprime =
            PriceToYield::f_prime(r[i], VN * ((0.01 * TC[i] * DPP) / YB), K[i], d[i]);

        EXPECT_GT(std::abs(fprime), 1e-12) << "f_prime near zero for r=" << r[i] << ", TC=" << TC[i]
                                           << ", K=" << K[i] << ", d=" << d[i];
    }
}
...
TEST(find_rootTest, BasicCase) {
    ...
    for (int i = 0; i < num_test; i++) {
        ...
        r_result = PriceToYield::find_root(VN * ((0.01 * TC[i] * DPP) / YB), K[i], d[i], P[i],
                                           &iters, precision);
        ...
        EXPECT_EQ(P_result, P_expected)
            << "Failed case " << i << " | TC=" << TC[i] << " K=" << K[i] << " d=" << d[i]
            << " r_true=" << r[i] << " r_found=" << r_result << " | P_expected=" << P_expected
            << " | P_result=" << P_result << " | diff=" << std::abs(P_result - P_expected);
    }
    auto end = high_resolution_clock::now();

    // performance metrics
    av_time =
        static_cast<double>(duration_cast<microseconds>(end - start).count()) / 1000.0 / num_test;
    av_diff = diff / num_test;
    av_iters = 1.0 * total_iters / num_test;
    failure_pct = 100.0 * failures / num_test;

    std::cout << "\n"
              << "SUMMARY | Precision: " << precision << " | Tests: " << num_test << "\n"
              << "==========================================" << "\n"
              << " | Avg diff: " << av_diff << " | Max diff: " << max_diff << "\n"
              << " | Avg iters: " << av_iters << " | Max iters: " << max_iters << "\n"
              << " | Avg time: " << av_time << " ms" << "\n"
              << "==========================================" << "\n"
              << " Fail count: " << failures << " | Failure rate: " << failure_pct << "%" << "\n\n";
}
...
TEST(find_rootTest, PrecisionSweep) {
    ...
    const std::vector<double> precision_values = {1e-11,   5e-11, 6e-11,   7e-11, 7.5e-11, 8e-11,
                                                  8.5e-11, 9e-11, 9.5e-11, 1e-10, 1.5e-10};
    ...
    for (auto precision_value : precision_values) {
        ...
        avg_time = static_cast<double>(duration_cast<microseconds>(end - start).count()) / 1000.0 /
                   num_test;
        avg_iters = 1.0 * total_iters / num_test;
        fail_rate = 100.0 * fail_count / num_test;

        std::cout << "\n"
                  << "SUMMARY | Precision: " << precision_value << " | Tests: " << num_test << "\n"
                  << "=========================================="
                     "\n"
                  << " | Avg diff: " << avg_diff << " | Max diff: " << max_diff << "\n"
                  << " | Avg iters: " << avg_iters << " | Max iters: " << max_iters << "\n"
                  << " | Avg time: " << avg_time << " ms" << "\n"
                  << "==========================================" << "\n"
                  << " Fail count: " << fail_count << " | Failure rate: " << fail_rate << "%"
                  << "\n\n";
    }
}
...
TEST(price_to_yieldTest, BasicCase) {
    ...
    for (int i = 0; i < num_test; i++) {
        ...
        yields[i] = PriceToYield::price_to_yield(P[i], dtms[i], TC[i]);

        // input yield result to px to compute theoretical prices and round
        P_result = PriceToYield::round_to_vec(px_vec(TC[i], yields[i], K[i], d[i]), 6);
        P_expected = P[i];
    ...
    // performance metrics
    av_time = static_cast<double>(duration_cast<microseconds>(end - start).count()) / 1000.0 /
              (num_test * num_mbonos);
    av_diff = diff / num_test;
    failure_pct = 100.0 * failures / num_test;

    std::cout << "\n"
              << "SUMMARY" << " | Tests: " << num_test << " (" << num_test * num_mbonos << " bonds)"
              << "\n"
              << "==========================================" << "\n"
              << " | Avg diff: " << av_diff << " | Max diff: " << max_diff << "\n"
              << " | Avg time: " << av_time << " ms" << "\n"
...
std::vector<double> px_vec(std::vector<double> TC_vec, std::vector<double> r_vec,
    ...
    for (int i = 0; i < TC_vec.size(); i++) {
        ...
        const int K = K_vec[i];
        const int d = d_vec[i];
        px_vec[i] = (C + C * (1 / R - 1 / (R * pow(1 + R, K - 1))) + VN / pow(1 + R, K - 1)) /
                        pow(1 + R, 1 - 1.0 * d / DPP) -
                    C * 1.0 * d / DPP;
    }
    return px_vec;
