...
def test_reorder_cetes_data():
    ...
    maturities_in_days = [int(test_object.CETES_MATURITY_MAP.get(tenor.get("idSerie")).split()[0])
                        if test_object.CETES_MATURITY_MAP.get(tenor.get("idSerie")).split()[1].lower() == "days"
                        else int(test_object.CETES_MATURITY_MAP.get(tenor.get("idSerie")).split()[0])*364 
                        for tenor in reordered_cetes_data ]
...
def test_get_labels_dates_yields():
    ...
    curve_labels, curve_dates, curve_yields = test_object.get_labels_dates_yields(cetes_data)
